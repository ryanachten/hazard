...
func newTestSim() (Simulation, error) {
	return NewSimulation(10, 10, configuration.SimulationConfig{
		CitizenCount: 0,
		Hazard: hazard.Config{
			DurationRange: bounds.PositiveRange{Min: 100, Max: 100},
			Probability:   0,
			Count:         0,
		},
...
func TestHazard_RadiusGrowsEachTick(t *testing.T) {
	...
	sim.Hazards = append(sim.Hazards, hazard.Hazard{
		ID:            uuid.New(),
		Type:          hazard.FireHazard,
		CreatedAt:     0,
		Duration:      100,
...
func TestHazard_RemovedAfterDuration(t *testing.T) {
	...
	require.Equal(t, pathfinding.CellOpen, sim.Grid.GetCell(pathfinding.Position{X: 5, Y: 5}),
		"origin cell must be restored to open after hazard removal")
	require.Equal(t, pathfinding.CellOpen, sim.Grid.GetCell(pathfinding.Position{X: 5, Y: 4}),
		"expanded cell at radius 1 must be restored to open")
	require.Equal(t, pathfinding.CellOpen, sim.Grid.GetCell(pathfinding.Position{X: 5, Y: 3}),
		"expanded cell at radius 2 must be restored to open")
}

func TestHazard_CreationViaTick(t *testing.T) {
	sim, err := NewSimulation(10, 10, configuration.SimulationConfig{
		CitizenCount: 0,
		Hazard: hazard.Config{
			DurationRange: bounds.PositiveRange{Min: 10, Max: 10},
			Probability:   1.0,
			Count:         5,
		},
		SafeZone: safezone.Config{
	...
	require.Contains(t, []hazard.Type{hazard.FireHazard, hazard.FloodHazard, hazard.LavaHazard}, h1.Type)
	...
	require.Equal(t, pathfinding.CellHazard, sim.Grid.GetCell(h1.Origin))
...
func TestHazard_BlocksCitizenPath(t *testing.T) {
	...
	safeZoneCells := []pathfinding.Position{
		{X: 3, Y: 3}, {X: 3, Y: 4},
		{X: 4, Y: 3}, {X: 4, Y: 4},
	}
	safeZoneLocations := make(map[pathfinding.Position]*safezone.SafeZone, len(safeZoneCells))
	for _, cell := range safeZoneCells {
		grid.UpdateCell(cell, pathfinding.CellSafeZone)
	...
	require.Equal(t, pathfinding.CellHazard, grid.GetCell(blockedCell))

	origPath := make([]pathfinding.Position, len(sim.Citizens[0].Path))
